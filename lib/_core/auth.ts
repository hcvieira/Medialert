  ...
  try {
    ...
    if (Platform.OS === "web") {
      if (typeof window !== "undefined") {
        ...
        console.log("[Auth] Web: session token from localStorage:", token ? "present" : "missing");
        return token;
      }
      return null;
    }
...
export async function setSessionToken(token: string): Promise<void> {
  try {
    // Web platform: save token in localStorage
    if (Platform.OS === "web") {
      if (typeof window !== "undefined") {
        window.localStorage.setItem(SESSION_TOKEN_KEY, token);
        console.log("[Auth] Web: session token saved to localStorage");
      }
      return;
    }
